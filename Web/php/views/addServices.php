                ...
                if (response.success && response.services.length > 0) {
                    ...
                    response.services.forEach((service, index) => {
                        ...
                        if (service.flight_types && service.flight_types.length > 0) {
                            ...
                            console.log("No flight types or empty array for service:", service.id);
                        }
                            ...
                            <div class="company-card" data-service-id="${service.id}">
                                <div class="company-thumbnail">
                                    <img src="${service.thumbnail_path}" 
                                        ...
                                        onerror="console.error('Image failed to load:', '${service.thumbnail_path}'); this.parentElement.innerHTML='<div style=\\'padding:50px;text-align:center;color:#999;background:#f5f5f5\\'>Image not available</div>';"
                                        onload="console.log('Image loaded successfully for ${service.company_name}');">
                                </div>
                                <div class="company-info">
                                    <div class="company-title"><b>${service.company_name || 'Unknown Company'}</b></div>
                                        <div class="company-desc">${service.service_description || 'No description available'}</div>
                                        <div class="company-meta">
                                            <span class="company-rating">
                                                <i class="fas fa-star"></i> 5.0 
                                                <span class="company-reviews">(0)</span>
                                            </span>
                                            ${lowestPrice ? `<span class="company-price">Rs. ${lowestPrice.toLocaleString()}</span>` : '<span class="company-price">Price not available</span>'}
                                        </div>
                                </div>
                            </div>
                        `;
                        ...
                        const addedCard = companyGrid.find(`[data-service-id="${service.id}"]`);
